<template>
    ...
    <div class="text-muted fw-bold mt-2 mb-2" style="display:flex">
      <div class="row">
        <div class="col-sm-12 col-md-12">
          ...
          <!--                    v-if="user.role" clearable></el-input>-->
          <el-select v-model="status" style="width:160px;margin-left:10px" placeholder="请选择审核状态" clearable>
  ...
  <!--  新增续约表单-->
  <div>
    <el-dialog v-model="dialogFormVisiblee" title="复职信息" align-center center class="" width="350"
               style="border-radius: 0.875rem 1rem;">
      <el-form :model="state.formData" class="" status-icon :rules="rules" ref="ruleFormRef">
        <el-form-item prop="reason" size="large" label="复职原因：">
  ...
  <!--  续约审核表单-->
  <div>
    <el-dialog v-model="dialogUpdateVisiblee" title="复职审核" align-center center class="" width="400"
               style="border-radius: 0.875rem 1rem;">
    ...
    <el-dialog v-model="dialogMoreVisible" title="审核详情" align-center center class="" width="600"
      ...
      <el-form :model="state.updateData" class="" status-icon :rules="rules"
        ...
        <el-timeline>
          ...
          <el-timeline-item icon="close" type="danger" size="large" :timestamp="state.updateData.reinDate"
          ...
          <el-timeline-item icon="check" type="success" size="large" :timestamp="state.updateData.reinDate"
...
const dialogUpdateVisiblee = ref(false)
...
const clearFormData = () => {
  ...
  dialogUpdateVisiblee.value = false;
...
const save = () => {
  ...
  proxy.$refs.ruleFormRef.validate((valid) => {
    if (valid) {
      ...
      //发送后台请求
      request.post('admin/reinstatement/save', state.formData).then(res => {
        if (res.code == '200') {
          ElNotification.success('申请成功，请等待管理员审核！')
        } else {
...
const update = () => {
  ...
  request.put('admin/reinstatement/update', state.updateData).then((res) => {
    try {
      if (res.code == 200) {
...
//加载
const load = () => {
  request.get('/admin/reinstatement/page', {
    params: {
